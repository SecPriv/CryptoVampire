...
/// Negates a [`Condition`]
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ConditionNot<U>(pub U);

impl<L, N, U> Condition<L, N> for ConditionNot<U>
where
    U: Condition<L, N>,
    L: Language,
    N: Analysis<L>,
{
    fn check(&self, egraph: &mut EGraph<L, N>, eclass: Id, subst: &Subst) -> bool {
        !self.0.check(egraph, eclass, subst)
    }
}

/// Conjunction of [`Condition`]s
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ConditionAnd<U, V>(pub U, pub V);

impl<L, N, U, V> Condition<L, N> for ConditionAnd<U, V>
where
    U: Condition<L, N>,
    V: Condition<L, N>,
    L: Language,
...
{
    fn check(&self, egraph: &mut EGraph<L, N>, eclass: Id, subst: &Subst) -> bool {
        self.0.check(egraph, eclass, subst) && self.1.check(egraph, eclass, subst)
    }
}

/// Disjunction of [`Condition`]s
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ConditionOr<U, V>(pub U, pub V);

impl<L, N, U, V> Condition<L, N> for ConditionOr<U, V>
where
    U: Condition<L, N>,
    V: Condition<L, N>,
    L: Language,
