...
impl<L: Language> ENodeOrVar<L> {
    /// Convert this `ENodeOrVar` into an `L` if it is an `ENode`.
    ///
    /// Returns `Some(L)` if the variant is `ENode`, or `None` otherwise.
    #[must_use]
    pub fn into_enode(self) -> Option<L> {
        match self {
            Self::ENode(e) => Some(e),
            _ => None,
        }
    ...
    /// Extracts a variable from this `ENodeOrVar` if it is a `Var`.
    ///
    /// Returns `Some(Var)` if the variant is `Var`, or `None` otherwise.
    #[must_use]
    ...
    /// Extracts a reference to the underlying `L` if this `ENodeOrVar` is an `ENode`.
    ///
    /// Returns `Some(&L)` if the variant is `ENode`, or `None` otherwise.
    #[must_use]
    pub fn as_enode(&self) -> Option<&L> {
        match self {
            Self::ENode(e) => Some(e),
            _ => None,
        }
...
impl<'a, L: Language> From<&'a [ENodeOrVar<L>]> for Pattern<L> {
    fn from(expr: &'a [ENodeOrVar<L>]) -> Self {
        let ast = expr.iter().cloned().collect();
        Self::new(ast)
    }
}
...
impl<U, L: Language> FromIterator<U> for Pattern<L>
where
    PatternAst<L>: FromIterator<U>,
{
    fn from_iter<T: IntoIterator<Item = U>>(iter: T) -> Self {
        Self::new(PatternAst::from_iter(iter))
    }
}
