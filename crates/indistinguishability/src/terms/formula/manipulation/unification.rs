...
impl Substitution {
    ...
    fn simple_add(&mut self, var: Variable, formula: RecFOFormula) {
...
pub fn mgu(f1: &RecFOFormula, f2: &RecFOFormula) -> Result<Substitution, UnificationError> {
    ...
    while let Some((t1_orig, t2_orig)) = equations.pop_front() {
        ...
        match (t1, t2) {
            ...
            (RecFOFormula::Var(v), term) => {
                unify_variable(v, term, &mut subst, &mut equations)?;
            }
            (term, RecFOFormula::Var(v)) => {
                unify_variable(v, term, &mut subst, &mut equations)?;
            }
            ...
            (RecFOFormula::App { .. }, RecFOFormula::Quantifier { .. })
            | (RecFOFormula::Quantifier { .. }, RecFOFormula::App { .. }) => {
                return Err(UnificationError::Mismatch);
            }
        }
    }
