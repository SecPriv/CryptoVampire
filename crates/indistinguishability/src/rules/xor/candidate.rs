...
pub fn mk_rwrites(pbl: &Problem, aenc: &XOr) -> impl Iterator<Item = Rewrite> {
...
fn mk_rewrite_regular(pbl: &Problem, aenc: &XOr) -> impl Iterator<Item = Rewrite> {
    pbl.functions()
        .iter_current()
...
fn mk_rewrite_one(_pbl: &Problem, aenc: &XOr, f: &Function) -> impl Iterator<Item = Rewrite> {
...
fn mk_static(_pbl: &Problem, aenc: &XOr) -> impl Iterator<Item = Rewrite> {
    let XOr {
        enc,
        pk,
        candidate_m,
