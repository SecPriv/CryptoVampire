    ...
    use super::{Environement, parsing_environement::{get_function, get_sort}, state::State};
    ...
    impl<'a, 'bump: 'a> Parsable<'bump, 'a> for ast::FindSuchThat<'a> {
        ...
        fn parse(
            ...
            let right = right.parse(env, bvars, state.to_symbolic(), Some(MESSAGE.as_sort().into()))?;
            ...
            // parse the rest
            let condition =
                condition.parse(env, bvars, state.to_symbolic(), Some(CONDITION.as_sort().into()))?;
            let left = left.parse(env, bvars, state.to_symbolic(), Some(MESSAGE.as_sort().into()))?;
            ...
            match self {
                ast::Application::ConstVar { span, content } => {
                    ...
                    .unwrap_or_else(|| get_function(env, *span, *content).and_then(|f| {
                        ...
                        let f_eval = match state.get_realm() {
                            Realm::Evaluated => f.as_term_algebra().maybe_get_evaluated().unwrap_or(f),
                            Realm::Symbolic => f
                        };
            ...
            match inner {
                ast::InnerAppMacro::Msg(app) | ast::InnerAppMacro::Cond(app) => {
                    ...
                    if let Some(es) = expected_sort {
                        let n_es_2 = SortProxy::from(n_es).unify(&es, &state).into_rr(*main_span)?;
