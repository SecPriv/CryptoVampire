...
pub fn generate<'bump>(
    ...
    let step = STEP.clone();
    let init = pbl.protocol.init_step().function().f([]);

    assertions.extend(
        [
            mforall!(s!0:step; {
                lt.f([init.clone(), s.clone()]) | meq(init.clone(), s)
            }),
            mforall!(s!0:step; {
                lt.f([s.clone(), s.clone()]) >> meq(s.clone(), init.clone())
            }),
            mforall!(s1!1:step, s2!2:step, s3!3:step; {
                (
                    lt.f([s1.clone(), s2.clone()]) &
                    lt.f([s2.clone(), s3.clone()])
                ) >> lt.f([s1, s3])
            }),
            ...
            mforall!(s1!1:step, s2!2:step; {
                lt.f([s1.clone(), s2.clone()]) |
                lt.f([s2.clone(), s1.clone()]) |
                meq(s1.clone(), s2.clone()) |
                (!happens.f([s1.clone()])) |
                (!happens.f([s2.clone()]))
            }),
            happens.f([init.clone()]),
        ]
        .into_iter()
        .map(Axiom::theory)
