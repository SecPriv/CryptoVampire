  ...
  perSystem =
    { self', pkgs, system, ... }:
    {
      packages = {
        ...
        # `indistinguishability` binary (packages.indistinguishability), runs
        ...
        doc = pkgs.stdenv.mkDerivation {
          ...
          nativeBuildInputs = with pkgs; [
            mdbook
            self'.packages.indistinguishability
          ];
          buildPhase = ''
            export HOME="$TMPDIR"
            mkdir -p "$out"
            # run the scheme docgen against the built binary (the `make doc` step)
            indistinguishability ./crates/indistinguishability/scheme/docgen.scm
